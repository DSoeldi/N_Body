...
def calc_analytical_force(r, m, epsilon):
    ...
    # r_all = norm(r) # for jit implementation
    r_all = np.sqrt(np.sum(r**2, axis=1))
    ...
    for i in range(n):
        ...
        if r_i > 0:
            force_magnitude = G * M_enc * m[i] / (r_i)**3 # how strong the force on particle i is
            forces[i] = -force_magnitude * r[i]  # which direction it points
